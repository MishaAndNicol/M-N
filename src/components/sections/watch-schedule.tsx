...
import { Bell, BellOff, CalendarHeart, ChevronDown, ChevronUp } from "lucide-react";
...
const NOTIFY_PREF_KEY = "mn-watch-schedule-notify";
...
export function WatchSchedule({ whoAmI, myName, otherName, myTimezone, otherTimezone }: Props) {
  ...
  // In-page confirmation banner ("proposal sent", "X proposed a time", ...) -
  // separate from an actual OS notification, since that one needs the
  // person to opt in first and this one doesn't.
  ...
  // Whether this browser is allowed to fire OS-level notifications for
  // this scheduler. Opt-in and per-device (localStorage), since
  // `Notification.requestPermission` only works from a real user gesture -
  // it can't be requested silently on mount.
  const [notifyEnabled, setNotifyEnabled] = useState(false);
  useEffect(() => {
    if (typeof window === "undefined" || !("Notification" in window)) return;
    setNotifyEnabled(window.localStorage.getItem(NOTIFY_PREF_KEY) === "1" && Notification.permission === "granted");
  }, []);
  ...
  // Tracks proposal/agreement ids we've already reacted to, so a fresh
  // Firestore snapshot on an unrelated field (or our own optimistic write
  // landing) doesn't re-fire a notification for something already seen.
  // `null` means "first snapshot since mount" - skipped on purpose so
  // opening the room doesn't immediately notify about everything already
  // sitting on the board.
  const seenProposalIdsRef = useRef<Set<string> | null>(null);
  const seenAgreedIdRef = useRef<string | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snap) => {
      ...
      const next = { ...EMPTY_SCHEDULE, ...(snap.data() as Partial<ScheduleState>) };

      const isFirstSnapshot = seenProposalIdsRef.current === null;
      const nextIds = new Set(Object.keys(next.proposals));

      if (!isFirstSnapshot) {
        for (const [id, proposal] of Object.entries(next.proposals)) {
          if (proposal.by === whoAmI) continue; // our own proposal, not theirs
          if (seenProposalIdsRef.current!.has(id)) continue;
          const label = `${formatDayLabelInZone(proposal.utcMillis, myTimezone!)}, ${formatTimeInZone(
            proposal.utcMillis,
            myTimezone!
          )} (your time)`;
          notify(`${otherName} proposed a time`, label);
        }
        if (next.agreedProposalId && next.agreedProposalId !== seenAgreedIdRef.current) {
          const agreed = next.proposals[next.agreedProposalId];
          if (agreed) {
            notify(
              "Time agreed!",
              `${formatDayLabelInZone(agreed.utcMillis, myTimezone!)}, ${formatTimeInZone(agreed.utcMillis, myTimezone!)}`
            );
          }
        }
      }

      seenProposalIdsRef.current = nextIds;
      seenAgreedIdRef.current = next.agreedProposalId;
      setSchedule(next);
    });
    return () => unsub();
    // `notify` intentionally excluded: it only reads `notifyEnabled` at
    // call time via closure, and including it would re-subscribe this
    // listener (dropping/reattaching the Firestore snapshot) on every
    // toggle of the notification switch.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connected, whoAmI, myTimezone, otherName]);

  function notify(title: string, body: string) {
    setToast(`${title} - ${body}`);
    if (notifyEnabled && typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
      new Notification(title, { body, tag: "mn-watch-schedule" });
    }
  }
  ...
  function propose(dateKey: string) {
    ...
    // Mark as already-seen before it round-trips through Firestore, so the
    // snapshot listener above doesn't treat our own write as a fresh
    // proposal "from them" once it comes back down.
    seenProposalIdsRef.current?.add(id);
    void writeSchedule({
      proposals: {
        [id]: {
          by: whoAmI,
    ...
    setToast(
      `Proposal sent - ${formatDayLabelInZone(utcMillis, myTimezone!)}, ${formatTimeInZone(utcMillis, myTimezone!)}`
    );
    setCountering(false);
  }
      ...
      <div className="flex w-full items-center gap-2 px-6 py-4">
        ...
        >
          <span className="flex min-w-0 items-center gap-2.5 text-sm">
            ...
            <span className={cn("truncate text-mist", needsAttention && "text-thread")}>{summary}</span>
          </span>
        </button>
        {typeof window !== "undefined" && "Notification" in window && (
          <button
            ...
            title={notifyEnabled ? "Notifications on for this device" : "Get a notification when a time is proposed"}
            className="shrink-0 rounded-full p-1.5 text-mist transition-colors hover:bg-thread/10 hover:text-thread"
          >
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -6 }}
            className="mx-6 mb-3 rounded-full bg-thread/10 px-4 py-2 text-xs text-thread"
          >
            {toast}
          </motion.div>
        )}
